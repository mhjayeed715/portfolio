...
export default function Navbar() {
  ...
  return (
    <>
      {/* ── 1. Centered Floating Capsule Navbar (Natural Smooth Physics & Translucent Liquid Glass) ── */}
      <motion.header
      ...
      >
        ...
        >
          <motion.nav
            ...
            className="flex items-center gap-2 py-2 px-2.5 sm:px-3 rounded-full liquid-glass transition-all duration-300"
          >
            ...
            <a
              ...
              className="flex items-center gap-2 px-2 py-1 rounded-full hover:bg-foreground/5 dark:hover:bg-white/10 transition-colors cursor-pointer group shrink-0"
            >
              <div className="w-8 h-8 rounded-full overflow-hidden border border-white/50 dark:border-white/20 shadow-xs bg-secondary shrink-0">
                <img
                  src="/profile21.png"
                  alt="Jayeed"
              ...
              <span className="font-display font-bold text-sm tracking-tight text-foreground pr-1">
                Jayeed
              </span>
            </a>
            ...
            <button
              ...
              className="md:hidden p-2 rounded-full text-foreground hover:bg-secondary cursor-pointer"
          ...
          {open && (
            <>
              ...
              <motion.div
                ...
                className="pointer-events-auto fixed top-20 left-4 right-4 z-50 md:hidden max-w-sm mx-auto"
              >
                <div className="p-3.5 rounded-3xl liquid-glass shadow-2xl space-y-1">
                  {navLinks.map((link) => {
                    const isActive = activeSection === link.href.replace('#', '')
                    return (
                      <a
                        ...
                        href={link.href}
                        onClick={(e) => handleLinkClick(e, link.href)}
                        className={`flex items-center justify-between px-4 py-2.5 text-sm font-medium rounded-2xl transition-colors cursor-pointer ${
                          isActive
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 w-full px-4 py-2.5 rounded-2xl liquid-glass-btn font-semibold text-sm cursor-pointer"
                    >
                      <span>Get in Touch</span>
      ...
      {/* ── 2. Independent Floating Theme Toggle in Top-Right Corner ── */}
